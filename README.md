...
Análisis exploratorio de datos (EDA) sobre las campañas de marketing telefónico de una institución bancaria portuguesa, cruzadas con las características demográficas de sus clientes. Proyecto re[...]
...
| Nulos en `cons_price_idx` (1.1%) y `euribor3m` (21.5%) | Imputados con la mediana **del mismo mes y año de contacto**, ya que son indicadores macroeconómicos de un periodo, no del cliente individual |
| `date` en formato texto español (`"2-agosto-2019"`) | Parseado a `datetime` con un diccionario de meses en español; de aquí se derivan `contact_month` y `contact_year`, que el enunciado menciona |
| `pdays = 999` | Es un código especial ("nunca contactado antes"), no un número real de días. Se mantiene tal cual (convención estándar del dataset) y se añade la columna booleana `contactado_previamente` |
| Columnas `latitude` / `longitude` | No aparecen descritas en el enunciado y sus coordenadas corresponden a EE. UU., no a Portugal. Todo indica que son datos sintéticos sin relación real con el negocio |
| Duplicados | Comprobado: 0 filas duplicadas |

### `customer-details.xlsx`
- Las 3 hojas (`2012`, `2013`, `2014`) se unieron en un único DataFrame, añadiendo una columna de control (`hoja_origen`) para poder verificar que el año de alta (`Dt_Customer`) coincide con la hoja.
...
Se cruzan por el identificador de cliente (`id`) con un `inner join`: de los 43.170 clientes de `customer-details.xlsx`, **170 no tienen ningún contacto de campaña asociado** y quedan fuera del dataset final.
...
Solo el **11,3%** de los clientes contactados (4.844 de 43.000) suscribió el depósito a plazo. Esto es clave: cualquier comparación de medias entre "yes" y "no" debe leerse teniendo en cuenta que el evento es raro.
...
Los clientes que suscriben hablan de media **551 segundos** (~9 min) frente a **220 segundos** (~3,5 min) de quienes no suscriben — un 150% más. Es un patrón lógico (una conversación larga suele indicar interés del cliente).
...
Si la campaña anterior con ese cliente terminó en éxito (`poutcome = success`), la probabilidad de que vuelva a suscribir ahora es del **65,3%**, muy por encima del 8,8% de los clientes sin contacto previo.
...
- **Estudiantes** (31,3%) y **jubilados** (25,2%) suscriben muchísimo más que la media (11,3%), probablemente porque ambos colectivos suelen tener menos deuda/menor necesidad de liquidez inmediata.
- **Blue-collar** (6,9%) y **servicios** (8,1%) son los perfiles con menor tasa de conversión.
- A más nivel educativo (`university.degree`, 13,7%) mejor tasa que educación básica (`basic.9y`, 7,8%), aunque destaca que el grupo `illiterate` (22,2%) es una excepción — probablemente por ser una muestra muy pequeña.
...
Los indicadores macroeconómicos (`euribor3m`, `emp_var_rate`, `nr_employed`) están fuertemente correlacionados entre sí (0,77–0,91), lo cual tiene sentido: todos reflejan el ciclo económico general.

![Mapa de correlación](visualizaciones/06_mapa_correlacion.png)

Sin embargo, **no se observa una estacionalidad clara mes a mes**: la tasa de suscripción fluctúa entre el 8,5% y el 14,3% sin un patrón estable a lo largo de 2015-2019, lo que sugiere que el contexto macroeconómico influyó pero no de forma determinante.
...
`Income`, `Kidhome`, `Teenhome` y `NumWebVisitsMonth` presentan diferencias insignificantes entre clientes que suscriben y los que no (todas las diferencias porcentuales están por debajo del 1%), y sus correlaciones con la suscripción son prácticamente nulas.
...
- `campaign` (número de contactos en esta campaña) tiene una relación negativa con la suscripción: a más intentos, menor tasa de éxito por contacto (2,05 de media en quienes suscriben vs 2,63 en quienes no).
